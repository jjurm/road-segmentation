...
def patch_accuracy_fn(y_hat, y):
    ...
    h_patches = y.shape[-2] // C.PATCH_SIZE
    w_patches = y.shape[-1] // C.PATCH_SIZE
    patches_hat = y_hat.reshape(-1, 1, h_patches, C.PATCH_SIZE, w_patches, C.PATCH_SIZE).mean((-1, -3)) > CUTOFF
    patches = y.reshape(-1, 1, h_patches, C.PATCH_SIZE, w_patches, C.PATCH_SIZE).mean((-1, -3)) > C.CUTOFF
